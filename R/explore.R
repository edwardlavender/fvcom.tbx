...
#'
#' @param field2d A dataframe that defines the variables for which model outputs will be explored. This must contain the following columns: 'cov2d', a character vector of variable names; 'mesh_type', a character vector of the mesh type ("element" or "node") across which each variable is structured; 'extension', a character vector which defines the extension of model files (see \code{\link[WeStCOMSExploreR]{extract}}); and 'vector_field', a logical vector defining whether or not that variable is a scalar (FALSE) or vector field (TRUE). To save plots directly to file (see \code{make_plot} and \code{png_param}), the dataframe should contain a column 'dir2load' which specifies the folder in which all the plots for each variable will be saved.
#' @param dat_ls A named list of dataframes, with one element for each \code{field2d$cov2d}. Each element should comprise a dataframe which defines the WeStCOMS file date names, hours, and mesh (node or element) IDs for model predictions will be extracted (see \code{\link[WeStCOMSExploreR]{extract}}).
...
#' @param match_mesh_around_nodes A dataframe with two columns named 'mesh' and 'index' which defines the index in WeStCOMS files (columns or sheets for 2d and 3d arrays respectively) which corresponds to each node cell (see \code{\link[WeStCOMSExploreR]{extract}}). This may be required if \code{field2d} contains variables that are resolved at nodes.
#' @param match_mesh_around_elements A dataframe with two columns named 'mesh' and 'index' which defines the index in WeStCOMS files (columns or sheets for 2d and 3d arrays respectively) which corresponds to each element cell (see \code{\link[WeStCOMSExploreR]{extract}}). This is may be required if \code{field2d} contains variables that are resolved at elements.
...
#' ############################
#' ############################
#' #### Example (2): Implement explore() for multiple variables
#'

...
#' @param sun_angle_param A list containing some essential parameters that are passed to \code{\link[WeStCOMSExploreR]{compute_field_sun_angle}} if sun angle is computed. This list should contain elements with the names: nodexy; tz; hours; degrees; dir2save; and verbose. These are described in \code{\link[WeStCOMSExploreR]{compute_field_sun_angle}}. All other parameters are for calculating sun angle are fixed or computed internally.
              ...
              FUN = function(day){
                ...
                if(!vector_field){
                  ...
                  if(ev != "sun_angle" | ev == "sun_angle" && compute_sun_angle == FALSE){
                    ...
                    # Then we'll compute this using compute_field_sun_angle():
                  } else if(ev == "sun_angle" & compute_sun_angle){
                    # Define matrix
                    sun_angle_mat2save <-
                      compute_field_sun_angle(nodexy = sun_angle_param$nodexy,
                                            date = date_current,

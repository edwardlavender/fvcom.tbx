...
#' @description The depth of WeStCOMS layers can be computed from the depth of the seabed below mean sea level (in a given location), the tidal elevation (at a given time and in a given location) and a vector of constants which adjusts for the relative position of each layer. If these values are known, \code{calc_layer_depth()} can be implemented to calculate the depth of Sigma layers. If these values are unknown, the function \code{\link[WeStCOMSExploreR]{compute_layer_depth}} should be used instead: this function first computes the appropriate values for specified times/locations and then uses these values to calculate depths.
#'
...
#'
#' #### Examples
#' calc_layer_depth(dat_siglev$siglev[1:10], dat_h$h[1], el = 0)
#' calc_layer_depth(dat_siglev$siglev[10], dat_h$h[1:10], el = 0)
#' calc_layer_depth(dat_siglev$siglev[1:10], dat_h$h[1:10], el = 1:10)
...
#' \dontrun{
#'  calc_layer_depth(dat_siglev$siglev[1:10], dat_h$h[1:3], el = 1:3)
#' }
#'

...
#' @param data The data to be plotted across the mesh ID. For scalar fields, i.e., \code{vector_field = FALSE}, this is a single dataframe with two columns: ID, a numeric/factor/character column specifying the ID of each mesh cell, i.e., a unique reference to each element or node that matches that in \code{"mesh"}; and fvcom, a numeric column specifying the value asigned to that cell by the hydrodynamic model. For vector fields, this should be a list with two elements: one called "udata" and one called "vdata"; each element in the list should contain a dataframe, as described above, that contain the u and v vector components resolved at each element respectively.
...
#'
#' @param element_xy (optional) For vector fields only, element_xy is matrix containing the coordinates of each element in the mesh. If this is not supplied, then these are obtained from the mesh object. However, if you are applying this function in a custom iterative proceedure, supplying this matrix will improve computational performance (especially for large meshes) because it does not have to be recalculated every time.
...
plot2dfield <-
  function(coastline,
    ...
    cols <- col.fn(length(pretty_vals))

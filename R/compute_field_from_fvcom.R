...
#' @description This function defines new hydrodynamic fields from WeStCOMS outputs, such as thermocline strength. To compute new fields, the user must specify a list of file names, each file consisting of an environmental array. These are loaded sequentially or in parallel into R by the function. For each array or combination of arrays, a user-supplied function is applied to compute a new field. The resultant field is either return or saved as an array (see Details).
#'
...
#' @param calc A function which takes in a list of arrays required to compute a new field for a particular day and computes a new fields. See \code{\link[WeStCOMSExploreR]{calc_thermocline}}, \code{\link[WeStCOMSExploreR]{calc_speed}}, \code{\link[WeStCOMSExploreR]{calc_direction}} for examples. A custom function can be provided in the same vein.
...
#' @param dir2save (optional) A string which defines the directory in which to save files. If provided, the function will save a .rds file with the specified name in the specified directory. If not provided, the function will return a list of arrays, with one element for each file.
...
#'
#' @return For each \code{source_file}, the function computes a new field comprising predictions for each cell in the original file. Arrays are either returned in a list to the workspace (with one element for each file) or, if \code{dir2save} is provided, saved to a user-supplied directory as a .rds file.
#'
#' @examples

...
## Install and load `WeStCOMSExploreR`

Let's begin by installing and loading the `WeStCOMSExploreR` package: 

```{r}
## Install development version of package
# devtools::install_github("https://github.com/edwardlavender/WeStCOMSExploreR/")

## Load package
library(WeStCOMSExploreR)

## View functions
# help(package = "WeStCOMSExploreR")
```
...
To use FVCOM outputs in `R`, the first step is to acquire FVCOM files. FVCOM files can be obtained from source or from a remote server. For file acquisition from a remote server, `WeStCOMSExploreR` includes the `thredds_download()` function which is designed to download WeStCOMS files from the SAMS thredds server.

From any FVCOM file, the following outputs are required for many `WeStCOMSExploreR` functions and can be obtained:
...
For each FVCOM file, we also need to extract and save the environmental arrays of interest prior to implementing `WeStCOMSExploreR` functions (see below). 

## Directory conventions  

The size of FVCOM files makes reading them in and out of programmes challenging. For this reason, `WeStCOMSExploreR` is designed to work with environmental arrays rather than the full FVCOM files. Specifically, many functions read in/out environmental arrays to create new environmental fields or to extract/explore environmental conditions. Hence, it is necessary to define a set of directories into which, for each FVCOM file, environmental arrays can be extracted and saved. The following directory names are recommended: 
...
# Create a folder for each environmental variable in a a specified directory (dir):
...
head(WeStCOMSExploreR::dat_trinodes, 3)

# Examine dat_nodexy
str(WeStCOMSExploreR::dat_nodexy)
head(WeStCOMSExploreR::dat_nodexy, 3)
```
...
For each date specified, for each environmental variable, the function will save a new .rds file in the appropriate folder in the supplied directory. (.rds files are the like the `R` equivalent of .mat files; they are used by WeStCOMSExploreR to maximise storage space efficiency and ensure file consistency.) For larger sequences of dates, this function can be implemented in parallel, but this is not necessary here.
...
More broadly, it is often instructive to examine the overall variation in environmental conditions over time or space. For 2-dimensional environmental fields (e.g., wind speed, surface temperature), `WeStCOMSExploreR::summarise_field_2d()`, `WeStCOMSExploreR::plot_field_2d()` and `WeStCOMSExploreR::explore_field_2d()` provide a convenient way to do this. Let's begin by calculating summary statistics across the area around Oban at a given point in time by using `WeStCOMSExploreR::summarise_field_2d()`. We will then proceed to map the environmental conditions over space for a single point in time using the `WeStCOMSExploreR::plot_field_2d()` function. `WeStCOMSExploreR::explore_field_2d()` is a broader function which can produce these plots and/or calculate summary statistics over space at multiple time points. 
...
We can now proceed to summarise the variation across nodes for each hour, or for all hours, in the array by supplying the functions of interest to the `funs` argument of `WeStCOMSExploreR::summarise_field_2d()`. Note that the elements of the list need to be named, with a name corresponding to the function being applied: 
```{r}
# We can now use the WeStCOMSExploreR::summarise_field_2d to calculate summary statistics
...
WeStCOMSExploreR::summarise_field_2d(data = sample2d, 
                                   row_specific = TRUE, 
...
These two steps can be combined into a single step using `WeStCOMSExploreR::fvcom_array_to_df()`, which takes a raw WeStCOMS array and returns a dataframe for plotting: 
```{r}
...
dat_temp <- WeStCOMSExploreR::fvcom_array_to_df(data = sample, 
                                                hour = 1, 
                                                layer = 1, 
...
This information can be passed to the `WeStCOMSExploreR::plot_field_2d()` function via the `data` argument. This function creates a map of the conditions resolved across the mesh. The key arguments are: `coastline`, a shapefile that is used to delineate the coastline on the map; `mesh`, which specifies the mesh to be plotted (either a mesh around nodes or one around elements); and `vector_field`, a logical input that specifies whether or not the variable is a vector field (here, `vector_field = FALSE` since temperature is a scalar variable). There are a lot of options for graphical customisation, which you can view via `?WeStCOMSExploreR::plot_field_2d()`. Here, we'll use the default options and plot temperatures resolved in the Oban area for a snapshot in time: 
```{r, fig.cap = "A map of spatial variation in temperature (degrees) around Oban (WeStCOMSExploreR::plot_field_2d() scalar example 1).", fig.height = 8, fig.width = 8}
...
```{r, fig.cap = "A map of spatial variation in tidal elevation (m) around Oban (WeStCOMSExploreR::plot_field_2d() scalar example 2).", fig.height = 8, fig.width = 8}
WeStCOMSExploreR::plot_field_2d(coastline = WeStCOMSExploreR::dat_coast_around_oban,
                              mesh = dat_mesh_around_nodes,
                              vector_field = FALSE,
...
```{r, fig.cap = "A map of spatial variation in sun angle (degrees) around Oban (WeStCOMSExploreR::plot_field_2d() scalar example 3).", fig.height = 10, fig.width = 10}
...
```{r, fig.cap = "A map of spatial variation in current velocity (m/s) around Oban (WeStCOMSExploreR::plot_field_2d() vector example 2).", fig.height = 8, fig.width = 8}
WeStCOMSExploreR::plot_field_2d(coastline = dat_coast_around_oban,
...
It is often desirable to move beyond snapshots of spatial variation at particular moments in time to consider the extent to which spatial variation changes through time. To this end, the `WeStCOMSExploreR::explore_field_2d()` function provides a mechanism to move over multiple variables and/or timesteps (days and hours) to calculate summary statistics and/or create plots. After this, timeseries of summary statistics can then be plotted to examine the relative scale of spatial and temporal variation, and multiple plots can be linked into animations which map how spatial variation in environmental conditions changes through time. 

To implement `WeStCOMSExploreR::explore_field_2d()`, it is necessary to define `fields2d`, a dataframe which contains the environmental variables and their associated properties for which to calculate summary statistics and/or make plots. You also need to specify the directory from which files can be loaded alongside various other arguments for model output extraction and to customise the summary statistics and plots that are produced.

```{r, fig.cap = "Maps of spatial variation in environmental conditions around Oban at multiple time points (WeStCOMSExploreR::explore_field_2d() examples).", fig.height = 10, fig.width = 10}
...
# Use WeStCOMSExploreR::explore_field_2d() to move over multiple variables and time points
...
The `prettyGraphics` package includes some additional functions for 3-dimensional, interactive visualisation of environmental conditions. 
...
## Apendix B: A simple MATLAB® script to define environmental arrays required for WeStCOMSExploreR
